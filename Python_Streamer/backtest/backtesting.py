...
from backtest.backtest_helper import compute_strategy_stats
from core.dbUtils import get_symbol_id
from core.cache import symbol_cache, r


class BacktestStrategy(ABC):
    @abstractmethod
    async def calculate_equity_curve(self, portfolio_dict, days_ago, initial_capital):
        raise NotImplementedError


class DefaultBacktestStrategy(BacktestStrategy):
    async def calculate_equity_curve(self, portfolio_dict, days_ago, initial_capital):
...
async def run_backtest(user_portfolio, benchmark, days_ago, client_id, strategy: BacktestStrategy | None = None):
    initial_capital = 10_000
    strategy = strategy or DefaultBacktestStrategy()

    try:
        test_df = await strategy.calculate_equity_curve(user_portfolio, days_ago, initial_capital)
        bench_df = await strategy.calculate_equity_curve(benchmark, days_ago, initial_capital)
    except Exception as e:
